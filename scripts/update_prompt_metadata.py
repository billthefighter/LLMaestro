...
def get_previous_version(file_path: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        previous_content = subprocess.check_output(
            ["git", "show", "HEAD:" + file_path], text=True, stderr=subprocess.DEVNULL
        )
...
def determine_version_bump(current: Dict[str, Any], previous: Optional[Dict[str, Any]]) -> str:
    ...
    if current.get("system_prompt") != previous.get("system_prompt") or current.get("user_prompt") != previous.get(
        "user_prompt"
    ):
        minor += 1
        patch = 0
    ...
    try:
        ...
        if "git_metadata" not in current_data:
            current_data["git_metadata"] = {
                "created": {"commit": commit_hash, "author": author},
                "last_modified": {"commit": commit_hash, "author": author},
            }
        else:
