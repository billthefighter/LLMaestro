...
def create_example_chains():
    ...
    group = GroupChain(
        chains=[
            SequentialChain(
                steps=[make_step("task_1a"), make_step("task_1b")],
                llm=llm
            ),
            SequentialChain(
                steps=[make_step("task_2a"), make_step("task_2b")],
                llm=llm
            )
        ],
        llm=llm
    ...
    # 5. Map Chain
    template_chain = SequentialChain(
        steps=[
            make_step("process_item"),
            make_step("validate_result")
        ],
        llm=llm
    )
    map_chain = MapChain(
        chain_template=template_chain,
        llm=llm
    )
    ...
    nested = GroupChain(
        chains=[
            ChordChain(
                parallel_steps=[
                    make_step("parallel_task_1"),
                    make_step("parallel_task_2")
                ],
            ...
            ReminderChain(
                steps=[
                    make_step("nested_step_1"),
                    make_step("nested_step_2")
                ],
                llm=llm
            )
        ],
