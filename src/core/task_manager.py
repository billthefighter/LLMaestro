...
class TaskManager:
    ...
    def _decompose_chunk(self, task: Task, decomp_config: DecompositionConfig) -> List[SubTask]:
        ...
        return [
            SubTask(id=str(uuid.uuid4()), type=task.type, input_data=chunk, parent_task_id=task.id) for chunk in chunks
        ]
    ...
    def _decompose_file(self, task: Task, decomp_config: DecompositionConfig) -> List[SubTask]:
        ...
        return [
            SubTask(id=str(uuid.uuid4()), type=task.type, input_data=file_data, parent_task_id=task.id)
    ...
    def _decompose_error(self, task: Task, decomp_config: DecompositionConfig) -> List[SubTask]:
        ...
        return [
            SubTask(id=str(uuid.uuid4()), type=task.type, input_data=error_data, parent_task_id=task.id)
    ...
    def _aggregate_dynamic(self, task_type: str, results: List[Any]) -> Any:
        ...
        try:
            # Create a local namespace with required imports
            namespace = {"List": List, "Dict": Dict, "Any": Any, "results": results}
