...
class SQLQueryParams(BaseModel):
    ...
    query: str = Field(description="The SQL query to execute. For read-only queries, use SELECT statements.")
    params: Optional[Dict[str, Any]] = Field(
        default=None, description="Optional parameters to bind to the query for parameterized queries."
    )
...
class SQLReadOnlyGuard(BasicFunctionGuard):
    ...
    def _execute_query(self, query: str, params: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            columns = result.keys()
            return_value = [dict(zip(columns, row, strict=False)) for row in result.fetchall()]
