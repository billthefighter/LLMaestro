...
class TokenBucket(BaseModel):
    ...
    # Usage tracking
    daily_usage: Dict[date, int] = Field(
        default_factory=dict,
        description="Daily token usage mapped by date"
    )
    
    # Bucket state
    minute_tokens: int = Field(
        default=0,
        description="Current number of tokens in the minute bucket"
    )
    last_refill_timestamp: float = Field(
        default_factory=lambda: datetime.now().timestamp(),
        description="Last time the buckets were refilled"
    )
    ...
    async def initialize(self) -> None:
        ...
        ) - timedelta(days=30)  # Keep last 30 days
        
        self.daily_usage = {
            date_key: usage 
            for date_key, usage in self.daily_usage.items() 
            if date_key >= cutoff.date()
        }
    ...
    def refill_minute_bucket(self, current_time: float) -> None:
        ...
        self.minute_tokens = min(
            self.minute_tokens + minute_tokens,
