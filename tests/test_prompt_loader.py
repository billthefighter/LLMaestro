...
from src.prompts.loader import AuthConfig, CacheEntry, PromptLoader, PromptSource
...
def test_add_remote_source():
    ...
    with patch("subprocess.run") as mock_run:
        ...
        assert any(str(s.url) == "https://example.com/prompts/test.yaml" for s in loader.remote_sources)
...
def test_refresh_remote_sources():
    ...
    with patch("subprocess.run") as mock_run:
        ...
        mock_run.return_value = Mock(stdout=MOCK_REMOTE_YAML, stderr="", returncode=0)
...
def test_failed_remote_load():
    ...
    with patch("subprocess.run") as mock_run:
        ...
        mock_run.side_effect = subprocess.CalledProcessError(1, "curl", output=b"", stderr=b"Error")
...
def test_invalid_remote_yaml():
    ...
    with patch("subprocess.run") as mock_run:
        ...
        mock_run.return_value = Mock(stdout="not a yaml file", stderr="", returncode=0)
...
def test_caching(temp_cache_dir):
    ...
    with patch("subprocess.run") as mock_run:
        ...
        loader = PromptLoader(cache_dir=temp_cache_dir, remote_sources=[url])
...
def test_stale_cache(temp_cache_dir):
    ...
    stale_entry = CacheEntry(
        content=MOCK_REMOTE_YAML, last_fetched=datetime.now() - timedelta(hours=2), etag="old_etag"
    )
...
def test_authenticated_request():
    ...
    with patch("subprocess.run") as mock_run:
        ...
        auth = AuthConfig(token="test_token", headers={"X-Custom": "value"})
        source = PromptSource(url="https://example.com/test.yaml", auth=auth)
