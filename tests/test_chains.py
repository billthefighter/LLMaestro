...
async def test_chain_step_execution():
    ...
    def output_transform(response: LLMResponse) -> LLMResponse:
        return LLMResponse(content=response.content.upper(), metadata=response.metadata)

    step = ChainStep[LLMResponse]("test_task", input_transform=input_transform, output_transform=output_transform)
...
async def test_sequential_chain():
    ...
    # Create chain
    steps = [ChainStep[LLMResponse]("first_task"), ChainStep[LLMResponse]("second_task")]
    chain = SequentialChain[LLMResponse](steps, llm=llm, prompt_loader=loader)
...
async def test_parallel_chain():
    ...
    # Setup mocks
    llm = AsyncMock(spec=BaseLLMInterface)
    llm.process.side_effect = [LLMResponse(content=f"response {i}", metadata={}) for i in range(3)]
...
async def test_chord_chain():
    ...
    chain = ChordChain[LLMResponse](parallel_steps, callback_step, llm=llm, prompt_loader=loader)
...
async def test_map_chain():
    ...
    # Setup mocks
    llm = AsyncMock(spec=BaseLLMInterface)
    llm.process.side_effect = [LLMResponse(content=f"mapped {i}", metadata={}) for i in range(3)]
    ...
    template_chain = SequentialChain[LLMResponse]([template_step], llm=llm, prompt_loader=loader)

    # Create map chain
    chain = MapChain[LLMResponse](template_chain, llm=llm, prompt_loader=loader)

    # Execute chain
    inputs = [{"input": "first"}, {"input": "second"}, {"input": "third"}]
...
async def test_chain_error_handling():
    ...
    step = ChainStep[LLMResponse]("test_task", retry_strategy={"max_retries": 1, "delay": 0})
    chain = SequentialChain[LLMResponse]([step], llm=llm, prompt_loader=loader)
...
async def test_reminder_chain():
    ...
    for (expected_args, expected_kwargs), real_call in zip(
        expected_calls, loader.format_prompt.call_args_list, strict=False
    ):
...
async def test_reminder_chain_with_transforms():
    ...
    def custom_transform(context: ChainContext, **kwargs) -> Dict[str, Any]:
        return {"prompt": kwargs["prompt"], "extra_context": "Additional information"}

    # Create chain
    steps = [
        ChainStep[LLMResponse]("step_1", input_transform=custom_transform),
        ChainStep[LLMResponse]("step_2", input_transform=custom_transform),
    ]
    chain = ReminderChain[LLMResponse](steps, llm=llm, prompt_loader=loader)
...
async def test_recursive_chain():
    ...
    def step_generator(response: LLMResponse, context: ChainContext) -> List[ChainStep[LLMResponse]]:
        if response.metadata.get("needs_followup"):
            return [ChainStep[LLMResponse]("followup_1"), ChainStep[LLMResponse]("followup_2")]
        return []
